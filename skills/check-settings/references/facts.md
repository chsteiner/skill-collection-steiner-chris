...
| F2 | `advisorModel` (Advisor-Tool) | schema | **Advisor muss mindestens so fähig sein wie das Main-Modell** (Advisor-Doku: „The advisor must be at least as capable as the main model"); ein schwächerer wird nicht angehängt, die Session läuft ohne. **Fable als Advisor ist derzeit in einer Rollout-Pause** (Picker zeigt `Fable 5 (temporarily unavailable)`, `/advisor fable` wird abgelehnt), betrifft **auch Accounts mit Fable-Zugang**; laut Doku kommt es zurück (kein Dauer-Ausschluss). Paarungen (Stand 2026-07-22): **Fable-Main → nur Fable-Advisor**, also aktuell **kein** Advisor; **Opus 4.7/4.8-Main → Opus 4.7/4.8** oder (pausiert) Fable, **Sonnet abgelehnt**; **Sonnet-5-Main → Sonnet 5/Opus/Fable**. Gültige Werte: `"opus"`, `"sonnet"`, volle Model-ID (`"fable"` erst nach Rollout). Advisor-Tool ist **experimentell, nur Anthropic-API**. **Heute-Alternative:** Subagent mit `model: fable` (dokumentiert gültig) an Entscheidungspunkten konsultieren, unabhängig vom Rollout. Verifiziert gegen `/docs/en/advisor` + `/docs/en/sub-agents` 2026-07-22 (die frühere „keine ≥-Regel"-Behauptung aus der Settings-Seite war unvollständig). | Advisor-Doku (`/docs/en/advisor`) |
